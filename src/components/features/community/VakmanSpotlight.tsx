...
export async function VakmanSpotlight({ data }: { data: VakmanSpotlightData }) {
  ...
  const v: VakmanRow = {
    id: vakman.id!,
    company_name: vakman.company_name!,
    slug: vakman.slug!,
    logo_url: vakman.logo_url,
    bio: vakman.bio,
    verified: vakman.verified ?? false,
    avg_score: Number(vakman.avg_score ?? 0),
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-1.5">
            <h4 className="font-bold text-body text-warmzwart truncate">{v.company_name}</h4>
            {v.verified && <SealCheck size={16} weight="fill" className="text-groen shrink-0" />}
          </div>
          {v.bio && <p className="text-body-sm text-warmgrijs truncate">{v.bio}</p>}
          <div className="flex items-center gap-1 mt-1">
            <Star size={14} weight="fill" className="text-oker" />
            <span className="text-body-sm font-semibold">{v.avg_score.toFixed(1)}</span>
